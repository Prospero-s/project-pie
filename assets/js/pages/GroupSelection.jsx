...
const GroupSelection = ({ i18n }) => {
    ...
    const useAuthHeaders = () => {
        const setupAuthHeaders = async () => {
            try {
                ...
                console.error('Erreur configuration headers:', error);
                return false;
            }
        };
    ...
    const loadPendingInvitations = async () => {
        try {
            ...
            setFormState(prevState => ({ ...prevState, pendingInvitations: response.data.invitations }));
        } catch (error) {
    ...
    const handleAcceptInvitation = async (token) => {
        try {
            setUiState(prevState => ({ ...prevState, isRedirecting: true }));
            const response = await axios.post(`/api/group-invitations/accept/${token}`);
            ...
            children: (
                <div className="mt-4 max-w-xl mx-auto">
                    <Card className="shadow-lg">
                        <Title level={4} className="mb-6">{t('create-group-title')}</Title>
        ...
        <div className="p-6">
            ...
            <div className="flex justify-center">
                <Tabs 
                    ...
                    onChange={(key) => setFormState(prevState => ({ ...prevState, activeTab: key }))}
